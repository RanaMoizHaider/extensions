...
export async function has_pending_changes(project?: string): Promise<boolean> {
  const s = await muxy.git.status({ local: true, project }).catch(() => null);
  if (!s) return false;
  return s.stagedFiles.length > 0 || s.unstagedFiles.length > 0;
}

export async function commit_all(message: string, project?: string): Promise<boolean> {
  return try_action(
